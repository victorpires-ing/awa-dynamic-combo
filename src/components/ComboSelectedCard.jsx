...
export default function ComboSelectedCard({ selected, combo, onEditar }) {
  const firstDate = selected.datas[0]?.data
  const sessionCount = selected.datas.length
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-0.5 min-w-0 flex-1">
        ...
        <p className="text-sm font-bold text-neutral-900 leading-5">{selected.nome}</p>
        {firstDate && (
